...
export default [
  {
    ...
    rules: {
      ...
      camelcase: ["warn", { properties: "never" }], // Warns if camelCase is not used for variable names
      ...
      "simple-import-sort/imports": [
        ...
        {
          groups: [
            // Node.js builtins (comes first)
            ["^(node:)?\\w"],

            // External packages (non-node, non-relative)
            ["^[^.]"],

            // Internal modules (aliased imports usually starting with @)
            ["^@\\/"],

            // Internal files (relative imports)
            ["^\\.\\/(?!.*\\.json$)"],
            ["^\\.\\.\\/((?!.*\\.json$))"],

            // Internal JSON files (relative .json imports)
            ["\\.json$"],
      ...
      ], // Enforces sorted imports to improve readability
      ...
      "no-restricted-syntax": [
        "error",
        {
          ...
          message:
            "Use inline exports (e.g., export class X {}) instead of separate export statements.",
        },
      ],
    },
  },
